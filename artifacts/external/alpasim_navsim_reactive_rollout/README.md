...
geometry, so AlpaBridge added and declared a flat `z=0` physics surface without
changing the recorded camera, map, or trajectory payloads.

## Result

- AlpaSim: `NVlabs/alpasim@9177bd0bec547d7516cc77d1864e943780ef7e7a`
- AlpaBridge run commit: `aece540bfc916bc9044ed6d4f0512d16093434a7`
...
fifth camera update, AlpaBridge returned `INVALID_ARGUMENT` because the ego pose
...
- `camera-map.mp4`: AlpaSim's raw 19.9-second run video; the map moves while the
  camera panel stays on the recorded seed frame

[Open the raw camera-and-map run video](camera-map.mp4). |
[Full-frame standalone preview](../../../docs/assets/readme/alpasim-closed-loop.gif) |
[Map-only trajectory preview](../../../docs/assets/readme/alpasim-trajectory-map.gif) |
[Map-only closed-loop-vs-logged divergence clip](../../../docs/assets/readme/alpasim-closed-loop-divergence.gif) |
[README hero (bottom half: this rollout's map + static camera)](../../../docs/assets/readme/alpasim-demo-two-rollouts.gif) |
[Camera side-by-side comparison, superseded by the hero above](../../../docs/assets/readme/alpasim-camera-comparison.gif).
...
Run the AlpaBridge external driver from the runtime image with the pinned
checkpoint mounted at `/checkpoint.ckpt`:

```bash
python -m alpabridge.challenge.e2e_driver \
