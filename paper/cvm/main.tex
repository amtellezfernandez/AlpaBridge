...
trajectory policies and AlpaSim external drivers. We evaluate a retained
\CVMExternalTraceDriveRPCs{}-call AlpaSim trace using \CVMDiagnosticFaultCases{}
single-fault mutations and \CVMDiagnosticControlCases{} valid prefix controls; expected
labels are withheld from the detector. WOD2Sim classifies
...
\CVMStatusDiagnosticCorrect{}/\CVMDiagnosticCases{} and detects no faults
(exact paired McNemar $p=\CVMDiagnosticMcNemarP{}$). Across 3,000 fault decisions,
median post-trace diagnosis is \CVMDiagnosisMedianUs{}~$\mu$s. In 200 paired
route-following measurements, online camera/context and freshness checks add
\CVMGuardOverheadMedianUs{}~$\mu$s median while preserving trajectory output, equivalent
to \CVMGuardOverheadTracePercent{}\% of the retained driver-call median. Separately,
the contract gate rejects all \CVMContractInvalidRejectedRuns{} completed,
metric-bearing command-only rollouts admitted by the status gate. The result is a
measured failure-attribution method; learned-policy quality and scenario coverage are
outside this study.
\end{abstract}
...
\begin{itemize}
  ...
  \item a controlled trace-mutation study measures localization, post-trace diagnosis
  time, and online guard overhead against an executable status-only gate.
\end{itemize}
...
The diagnostic experiment starts from a hashed, retained external AlpaSim trace with
\CVMExternalTraceDriveRPCs{} drive calls. Fifteen single-field or runtime-context
mutations cover three semantic, three temporal, three lifecycle, one plugin, three
deployment, and two evidence faults. Fifteen increasing trace prefixes, each terminated
by the retained close event, serve as valid controls. Mutation construction and
diagnosis are separate: the detector receives only mutated events and runtime context;
the scorer retains the expected label. For a paired comparator, define the executable
status-only gate
\begin{equation}
B_{\mathrm{status}}(r)=
\mathbf{1}[\text{completed}(r)\land\text{metrics}(r)].
\end{equation}
Both methods return a decision for all 30 cases. We report correct classification, fault recall,
localization, control false positives, Wilson 95\% intervals, and an exact two-sided
McNemar test over paired correctness.

Post-trace timing uses 200 randomized iterations, five calls per batch, yielding 3,000
fault-diagnosis and 6,000 all-case WOD2Sim measurements. A second 200-iteration paired
ablation compares the same route-following prediction with camera shape/context and
sensor-freshness guards enabled or disabled, randomized within each iteration and
batched over 20 calls. Both paths retain route extraction, trajectory generation,
resampling, and reasoning serialization; output trajectories must match exactly.
Completed simulator rows are separately audited for route source and sensor freshness.
The same status gate is applied to metric-bearing command-only rows, while WOD2Sim
additionally requires all five contracts.
...
Controlled-trace results appear in Table~\ref{tab:diagnostic-comparison}. WOD2Sim is
...
completed fault case; it detects no faults. All 15 discordant pairs favor WOD2Sim
(exact two-sided McNemar $p=\CVMDiagnosticMcNemarP{}$). The Wilson 95\% lower bounds
are 0.886 for case classification and 0.796 for fault recall, making the finite
denominators explicit.

\begin{table}[t]
\centering
\caption{Paired controlled-trace diagnostics. FP denotes a valid control classified
as faulty.}
...
Median WOD2Sim decision time is \CVMDiagnosticDecisionMedianUs{}~$\mu$s over 6,000
measurements; correct fault diagnosis is \CVMDiagnosisMedianUs{}~$\mu$s over 3,000 fault
measurements. The paired online guard increment is \CVMGuardOverheadMedianUs{}~$\mu$s, or
\CVMGuardOverheadRelativePercent{}\% relative to the deliberately small unchecked
route-following path. For operational context, it is
\CVMGuardOverheadTracePercent{}\% of the retained external driver-call median
(\CVMExternalTraceLatencyMedianMs{}~ms); all
\CVMExternalChallengeLatencyTargetMet{}/\CVMExternalChallengeLatencyTargetDenominator{}
retained calls meet the 100-ms target.
...
mutation set. Its 15 clean prefixes supply a negative class, so the observed
0/\CVMDiagnosticControlCases{} false-positive count is independently scored rather
than defined by audit admission. The closed-loop rows test a second path: adding
route-source validity changes the attribution of all
\CVMStatusOnlyAcceptedContractRejectedRuns{} metric-bearing command-only records
without changing their simulator outcome.

The timing results separate online and post-run costs. The
\CVMGuardOverheadMedianUs{}~$\mu$s value measures only camera/context and freshness checks
inside prediction; the \CVMDiagnosisMedianUs{}~$\mu$s value measures in-memory diagnosis
after telemetry parsing. A missing actor proxy or checkpoint remains a deployment
precondition, a command proxy a semantic violation, and missing audit state an evidence
violation. Typed outcomes keep those cases available for debugging while excluding them
from policy attribution.
...
The study scope is contract conformance. The mutations are framework-authored and
applied offline to one retained trace; the valid controls are correlated prefixes of
that trace. The status-only gate is a concrete completion-and-metrics comparator, not a
surrogate for CARLA, nuPlan, or another full framework. The repository is not an
official WOMD interface, does not reconstruct full WOMD worlds, and has no public
learned checkpoint, scene-matched actor proxy, or verified scenario-category coverage.
Restricted scenes remain licensed inputs; public artifacts retain the trace, manifests,
...
diagnosis are separate functions, expected labels are withheld from the detector, every
case is scored afterward, and 15 unmutated controls test false alarms. McNemar's test is
paired by case; it measures the observed difference against the status gate on this
designed set, not a population prevalence. Timing order is randomized and paired on one
host, so hardware and background-load effects remain shared measurement conditions.

\emph{Construct validity.} The online overhead ablation covers camera shape/context and
freshness checks in dependency-light route following; the diagnosis timer begins after
JSON parsing. All five contracts, including the plugin precondition within deployment,
are mutated against retained telemetry and context rather than rerun as physical
simulator faults. Contract-valid therefore means admitted
...
availability and remain category-unclassified. The diagnostic cases share one external
trace, and the closed-loop route pairs have one execution per arm with no effective
seed control. Accordingly, the reported comparison concerns the declared mutations and
status gate; the measured latencies concern this host and dependency-light policy path.
Learned-policy behavior, rare-event coverage, and route-loss effects require separate
experiments.
...
deployment, and evidence contracts. On 15 controlled faults and 15 valid controls it
classifies 30/30 cases, localizes 15/15 faults, and differs from the status-only gate on
all 15 fault cases. Post-trace diagnosis and online guard costs are measured rather than
assumed. Together with the retained closed-loop route invalidations, the result shows
