...
class TokenBCAlpaSimModel(BaseTrajectoryModel):
    ...
    def from_config(
        ...
        trajectory_mode = os.getenv("ALPABRIDGE_TOKENBC_TRAJECTORY_MODE", str(_cfg_value(model_cfg, "trajectory_mode", "token")))
        max_lateral_offset_m = float(
            os.getenv("ALPABRIDGE_TOKENBC_MAX_LATERAL_OFFSET_M", str(_cfg_value(model_cfg, "max_lateral_offset_m", 2.0)))
        )
        selection_mode = os.getenv("ALPABRIDGE_TOKENBC_SELECTION_MODE", str(_cfg_value(model_cfg, "selection_mode", "argmax")))
        hybrid_top_k = int(os.getenv("ALPABRIDGE_TOKENBC_HYBRID_TOP_K", str(_cfg_value(model_cfg, "hybrid_top_k", 3))))
        hybrid_geo_weight = float(
            os.getenv("ALPABRIDGE_TOKENBC_HYBRID_GEOMETRIC_WEIGHT", str(_cfg_value(model_cfg, "hybrid_geometric_weight", 0.75)))
        )
        hybrid_policy_temperature = float(
            os.getenv(
                "ALPABRIDGE_TOKENBC_HYBRID_POLICY_TEMPERATURE",
                str(_cfg_value(model_cfg, "hybrid_policy_temperature", 1.0)),
            )
        )
        hybrid_veto_margin = float(
            os.getenv("ALPABRIDGE_TOKENBC_HYBRID_VETO_MARGIN", str(_cfg_value(model_cfg, "hybrid_veto_margin", 8.0)))
        )
        hybrid_max_geometric_rank = int(
            os.getenv(
                "ALPABRIDGE_TOKENBC_HYBRID_MAX_GEOMETRIC_RANK",
                str(_cfg_value(model_cfg, "hybrid_max_geometric_rank", 2)),
            )
        )
        selection_log_path = os.getenv("ALPABRIDGE_TOKENBC_SELECTION_LOG_PATH", str(_cfg_value(model_cfg, "selection_log_path", "")))
        oracle_actor_proxy_path = os.getenv(
            "ALPABRIDGE_TOKENBC_ORACLE_ACTOR_PROXY_PATH",
            str(_cfg_value(model_cfg, "oracle_actor_proxy_path", "")),
        )
        oracle_actor_proxy_tolerance_us = int(
            os.getenv(
                "ALPABRIDGE_TOKENBC_ORACLE_ACTOR_PROXY_TOLERANCE_US",
    ...
    def predict(self, prediction_input: PredictionInput) -> ModelPrediction:
        ...
        reasoning_text = json.dumps(
            {
                "adapter": "alpabridge.simulator.alpasim_token_bc",
