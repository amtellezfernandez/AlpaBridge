...
from alpabridge.cli.runtime_paths import workspace_path
...
async def _build_proxy(args: argparse.Namespace, asl_files: list[Path]) -> dict[str, Any]:
    try:
        ...
        raise SystemExit(
            ...
            "  ALPASIM_ROOT=/path/to/alpasim alpabridge-build-oracle-proxy --run-dir /path/to/run --output oracle.json"
        ) from exc
