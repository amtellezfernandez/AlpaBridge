...
from alpabridge.cli.commands.check_alpasim_readiness import (
...
from alpabridge.cli.runtime_paths import SOURCE_REPO_ROOT, package_path, repo_path, workspace_path
...
EXPECTED_CONSOLE_SCRIPTS = (
    ...
    "alpabridge-audit-run",
    "alpabridge-support-bundle",
    "alpabridge-reproduce",
    "alpabridge-promote-batch-summary",
    "alpabridge-benchmark-summary",
...
EXPECTED_WRAPPERS = {
    ...
    "alpabridge-setup": "scripts/setup_alpasim_local_plugin.py",
    "alpabridge-ready": "scripts/check_alpasim_readiness.py",
    "alpabridge-launch": "scripts/run_alpasim_local_external.py",
    "alpabridge-batch": "scripts/run_alpasim_scene_batch.py",
    "alpabridge-build-local-cache": "scripts/build_alpasim_local_usdz_cache.py",
    "alpabridge-build-oracle-proxy": "scripts/build_alpasim_oracle_actor_proxy.py",
    "alpabridge-register-custom-scene": "scripts/register_alpasim_custom_scene.py",
    "alpabridge-audit-run": "scripts/audit_run.py",
    "alpabridge-support-bundle": "scripts/support_bundle.py",
    "alpabridge-reproduce": "scripts/reproduce_closed_loop.py",
    "alpabridge-promote-batch-summary": "scripts/promote_batch_summary.py",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate the public AlpaBridge release surface before wiring it into AlpaSim."
    )
    parser.add_argument(
        "--alpasim-root",
        type=Path,
        default=None,
        help="Optional AlpaSim checkout root to validate alongside the AlpaBridge release surface.",
    )
    parser.add_argument(
        "--scene-preset",
    ...
    parser.add_argument(
        "--skip-camera-rig-check",
        action="store_true",
        help="Skip cross-checking model presets against local ego-hood rig camera masks.",
    )
    parser.add_argument(
...
def _build_environment_report(
    *,
    ...
    if skip_camera_rig_check:
        statuses["camera_rig_compatibility"] = "skipped"
    elif statuses["alpasim_checkout"] != "ok":
        statuses["camera_rig_compatibility"] = "blocked"
        errors["camera_rig_compatibility"] = (
            "AlpaSim checkout validation failed; camera rig check not run."
        )
    else:
        status, error = _run_check(_preflight_camera_rig_compatibility, alpasim_root=alpasim_root)
        record("camera_rig_compatibility", status, error)
...
def build_report(
    *,
    ...
    skip_camera_rig_check: bool = False,
    ...
    for name, entries in public_model_entry_point_providers.items():
        ...
        for entry in entries:
            if _normalized_distribution_name(entry["dist"]) != "alpabridge":
    ...
    checks = {
        ...
        "public_model_entry_points_owned_by_alpabridge": not unexpected_public_model_providers,
    ...
    if requested_alpasim_root is not None:
        environment = _build_environment_report(
            ...
            skip_camera_rig_check=skip_camera_rig_check,
        )

    report = {
        "schema": "alpabridge_doctor_v1",
        "valid": bool(
            ...
            and checks["public_model_entry_points_owned_by_alpabridge"]
...
def _print_human_report(report: dict[str, object], *, strict_installed: bool) -> None:
    ...
    if any(conflicts.values()):
        ...
        print("    3. If this is an AlpaSim plugin env, rerun alpabridge-setup --alpasim-root /path/to/alpasim")
        print("    4. Then rerun alpabridge-doctor")
        return
    if environment is None:
        print("    2. Run alpabridge-doctor --alpasim-root /path/to/alpasim")
        print("    3. Run alpabridge-ready --alpasim-root /path/to/alpasim")
    else:
        ...
        if environment["statuses"].get("alpasim_checkout") == "failed":
            ...
            print("    4. Then rerun alpabridge-doctor --probe-default-environment")
            print("    5. After the checkout exists, run alpabridge-ready --alpasim-root /path/to/alpasim")
            ...
            print("    5. Then rerun alpabridge-ready --alpasim-root /path/to/alpasim")
            ...
            print(
                "    5. After the cache issue is fixed, run alpabridge-launch --mode print "
                "--model token_dagger_bc --checkpoint /path/to/token_dagger_bc.pt"
            )
        else:
            print(
                "    3. Run alpabridge-launch --mode print --model token_dagger_bc "
...
def main() -> int:
    ...
    report = build_report(
        ...
        skip_camera_rig_check=args.skip_camera_rig_check,
