...
SUMMARY_SCHEMA = "alpabridge_benchmark_summary_v1"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate one or more alpabridge-reproduce evidence directories into a compact "
            "benchmark summary."
        )
    )
    parser.add_argument(
...
def build_summary(
    *,
    ...
    return {
        ...
        "claim_rule": (
            "Every included run must be an executed alpabridge-reproduce run with "
...
def _print_human_summary(summary: dict[str, Any]) -> None:
    ...
    print("AlpaBridge benchmark summary")
