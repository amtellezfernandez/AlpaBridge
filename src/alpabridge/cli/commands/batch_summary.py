...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a compact, public-safe summary from an alpabridge-batch directory. "
            "The summary records metrics, failures, and hashes without embedding gated media."
        )
    )
    parser.add_argument(
        "--batch-dir", type=Path, default=None, help="Directory created by alpabridge-batch."
    )
    parser.add_argument(
        "--batch-status",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Public-safe alpabridge-batch-summary JSON to merge. Repeat for each shard. "
...
def _claim_boundary(
    *,
    ...
    source = (
        "a merged public-safe summary built from shard-level AlpaBridge closed-loop batch summaries"
        if merged
...
def _print_human_summary(summary: dict[str, Any]) -> None:
    ...
    print("AlpaBridge closed-loop batch summary")
