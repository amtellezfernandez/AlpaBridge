...
from alpabridge.cli.runtime_paths import package_path, workspace_path
...
MODEL_PRESETS = {
    ...
    "direct_actor_planner": {
        ...
        "force_cuda": False,
        "driver_env": {
            "ALPABRIDGE_DIRECT_PLANNER_ORACLE_ACTOR_PROXY_PATH": "{oracle_actor_proxy_path}",
            "ALPABRIDGE_DIRECT_PLANNER_ORACLE_ACTOR_PROXY_TOLERANCE_US": "50000",
            "ALPABRIDGE_DIRECT_PLANNER_LOG_PATH": "{run_dir}/driver/direct-planner-log.jsonl",
        },
    },
}
...
def _preflight_camera_rig_compatibility(
    *, alpasim_root: Path, models: tuple[str, ...] = PUBLIC_RELEASE_MODELS
) -> None:
    """Cross-check each preset's declared cameras against local ego-hood rig masks.

    AlpaSim's camera rig is defined by ego-hood mask assets under
    data/nre-artifacts/ego-hoods/<rig>/<camera_id>.png, independent of any
    particular scene: a preset asking for a camera no local rig has a mask
    for will fail _validate_cameras at predict() time on every scene, not
    just some. Catching that here, before a live AlpaSim session, is
    cheaper -- and the fix is a different upstream AlpaSim rig asset, not
    an adapter configuration change.
    """
    ego_hoods_dir = alpasim_root / "data" / "nre-artifacts" / "ego-hoods"
    if not ego_hoods_dir.is_dir():
        return
    rig_dirs = sorted(path for path in ego_hoods_dir.iterdir() if path.is_dir())
    if not rig_dirs:
        return

    available_cameras: set[str] = set()
    for rig_dir in rig_dirs:
        available_cameras.update(mask_file.stem for mask_file in rig_dir.glob("*.png"))

    problems: list[str] = []
    for model in models:
        config_file = MODEL_PRESETS[model]["config_file"]
        if not config_file.is_file():
            continue
        payload = yaml.safe_load(config_file.read_text()) or {}
        requested = list((payload.get("inference") or {}).get("use_cameras") or [])
        missing = [camera_id for camera_id in requested if camera_id not in available_cameras]
        if missing:
            problems.append(f"{model}: {', '.join(missing)}")

    if not problems:
        return
    rig_names = ", ".join(rig_dir.name for rig_dir in rig_dirs)
    raise SystemExit(
        "Model preset(s) request cameras no local ego-hood rig defines a mask for: "
        f"{'; '.join(problems)}. Rigs checked under {ego_hoods_dir}: {rig_names}. "
        f"Cameras available across all of them: {', '.join(sorted(available_cameras)) or 'none'}. "
        "This is an AlpaSim ego-vehicle rig asset limitation, not an adapter "
        "configuration error -- resolving it requires a different upstream rig "
        "asset with more camera masks, not a change to this repository's presets."
    )
...
def _preflight_platform_compatibility(deploy_target: str | None = None) -> None:
    ...
    raise SystemExit(
        ...
        "see AlpaSim's docs/VIDEO_MODEL.md.\n"
        "\n"
        f"AlpaBridge ships a deploy config for exactly this: set\n"
        f"  ALPABRIDGE_ALPASIM_DEPLOY_TARGET={VIDEO_MODEL_DEPLOY_TARGET}\n"
        "and supply the renderer address, e.g.\n"
...
def _planned_run_status(
    *,
    ...
    return {
        "schema": "alpabridge_run_status_v1",
        "state": "planned",
        "phase": "planned",
        "mode": str(args.mode),
...
def _wizard_deploy_target() -> str:
    override = os.getenv("ALPABRIDGE_ALPASIM_DEPLOY_TARGET", "").strip()
    if override:
        return override
    machine = platform.machine().lower()
