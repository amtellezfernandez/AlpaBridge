...
MANIFEST_SCHEMA = "alpabridge_closed_loop_reproduction_v1"
CLAIM_BOUNDARY = (
    "A valid executed reproduction requires a real AlpaSim checkout, local/gated scene assets, "
    "any model-specific checkpoint or oracle proxy, and a completed alpabridge-launch --mode both run. "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run or plan the AlpaBridge trajectory-policy to AlpaSim closed-loop reproduction workflow."
        )
    )
    parser.add_argument(
        "--execute",
    ...
    parser.add_argument(
        "--skip-setup",
        action="store_true",
        help="Skip alpabridge-setup when the AlpaSim environment is already wired.",
    )
        ...
        steps.append(
            {
                ...
                "command": [
                    sys.executable,
                    "-m",
                    "alpabridge.cli.commands.setup_alpasim_local_plugin",
                    "--alpasim-root",
                    str(alpasim_root),
                ],
            }
    ...
    steps.append(
        {
            ...
            "command": [
                sys.executable,
                "-m",
                "alpabridge.cli.commands.audit_run",
                "--run-dir",
                str(run_dir),
                "--audit-dir",
    ...
    steps.append(
        {
            ...
            "command": [
                sys.executable,
                "-m",
                "alpabridge.cli.commands.support_bundle",
                "--run-dir",
                str(run_dir),
                "--output",
...
def _provenance() -> dict[str, Any]:
    return {
        ...
        "alpabridge_version": _package_version(),
        "git": {
...
def _print_human_manifest(manifest: dict[str, Any]) -> None:
    print("AlpaBridge closed-loop reproduction")
