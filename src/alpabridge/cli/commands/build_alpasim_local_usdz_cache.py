...
MANIFEST_NAME = "alpabridge-local-usdz-cache-manifest.json"
...
def main() -> int:
    ...
    if source_usdz_dir is not None:
        if args.dry_run:
            ...
            report["schema"] = "alpabridge_local_usdz_source_cache_plan_v1"
    ...
    if args.dry_run:
        print(
            json.dumps(
                {
                    "schema": "alpabridge_local_usdz_cache_plan_v1",
                    "scene_preset": args.scene_preset,
...
def validate_local_usdz_cache(
    *,
    ...
    return {
        "schema": "alpabridge_local_usdz_cache_validation_v1",
        "valid": valid,
...
def _hf_available_paths(*, repo_id: str, revision: str, token: str | None) -> set[str]:
    try:
        ...
        raise SystemExit(
            ...
            "`pip install 'alpabridge[alpasim]'` or run inside the AlpaSim virtualenv."
        ) from exc
