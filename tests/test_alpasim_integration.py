...
from alpabridge.neutral.alpasim_metrics import build_alpasim_evidence, load_alpasim_metrics
...
from alpabridge.simulator.alpasim_signal import extract_alpasim_signal, scenario_from_command
from alpabridge.simulator.alpasim_token_bc import (
    TOKEN_ORDER,
    TokenBCAlpaSimModel,
...
class AlpaSimIntegrationTests(unittest.TestCase):
    def test_pyproject_registers_alpasim_plugin_entrypoints(self) -> None:
        ...
        self.assertEqual(
            ...
            "alpabridge.simulator.baseline_drivers:ConstantVelocityAlpaSimModel",
        )
        self.assertEqual(
            pyproject['project.entry-points."alpasim.models"']["route_following"],
            "alpabridge.simulator.baseline_drivers:RouteFollowingAlpaSimModel",
        )
        self.assertEqual(
            pyproject['project.entry-points."alpasim.models"']["token_dagger_bc"],
            "alpabridge.simulator.alpasim_token_bc:TokenBCAlpaSimModel",
        )
        self.assertEqual(
            pyproject['project.entry-points."alpasim.models"']["direct_actor_planner"],
            "alpabridge.simulator.alpasim_direct_actor_planner:DirectActorPlannerAlpaSimModel",
        )
        self.assertEqual(
            pyproject['project.entry-points."alpasim.configs"']["alpabridge"],
            "alpabridge.simulator.alpasim_configs",
        )
        self.assertEqual(
            pyproject["project.scripts"]["alpabridge-build-local-cache"],
            "alpabridge.cli.commands.build_alpasim_local_usdz_cache:main",
        )
        self.assertEqual(
            pyproject["project.scripts"]["alpabridge-build-oracle-proxy"],
            "alpabridge.cli.commands.build_alpasim_oracle_actor_proxy:main",
        )
        self.assertEqual(
            pyproject["project.scripts"]["alpabridge-audit-run"],
            "alpabridge.cli.commands.audit_run:main",
        )
        self.assertEqual(
            pyproject["project.scripts"]["alpabridge-support-bundle"],
            "alpabridge.cli.commands.support_bundle:main",
        )
        self.assertEqual(
            pyproject["project.scripts"]["alpabridge-promote-batch-summary"],
            "alpabridge.cli.commands.promote_batch_summary:main",
        )

    def test_direct_actor_planner_config_exists(self) -> None:
        config_path = Path("src/alpabridge/simulator/alpasim_configs/driver/direct_actor_planner.yaml")
    ...
    def test_alpasim_driver_configs_are_public_release_templates(self) -> None:
        config_dir = Path("src/alpabridge/simulator/alpasim_configs/driver")
        self.assertEqual(
            [
    ...
    def test_route_following_command_only_ablation_ignores_route_geometry(self) -> None:
        ...
        old_value = os.environ.get("ALPABRIDGE_ROUTE_CONTRACT_MODE")
        os.environ["ALPABRIDGE_ROUTE_CONTRACT_MODE"] = "command_only_route"
        try:
            output = model.predict(prediction_input)
        finally:
            if old_value is None:
                os.environ.pop("ALPABRIDGE_ROUTE_CONTRACT_MODE", None)
            else:
                os.environ["ALPABRIDGE_ROUTE_CONTRACT_MODE"] = old_value
        reasoning = json.loads(output.reasoning_text)
    ...
    def test_baseline_driver_accepts_more_than_one_camera(self) -> None:
        # camera_ids is a generic list; shipped presets happen to declare one
        # entry, but nothing in predict()/_validate_cameras is hardcoded to a
        # single camera. This proves the adapter contract itself scales to a
        # multi-camera rig, independent of what any single scene provides.
        model = RouteFollowingAlpaSimModel(
            camera_ids=["camera_front_wide_120fov", "camera_front_left_45fov"],
            context_length=1,
            output_frequency_hz=4,
        )
        prediction_input = SimpleNamespace(
            camera_images={
                "camera_front_wide_120fov": [
                    SimpleNamespace(timestamp_us=1000, image=np.full((4, 4, 3), 180, dtype=np.uint8))
                ],
                "camera_front_left_45fov": [
                    SimpleNamespace(timestamp_us=1000, image=np.full((4, 4, 3), 90, dtype=np.uint8))
                ],
            },
            command=DriveCommand.STRAIGHT,
            speed=4.0,
    ...
    def test_baseline_driver_camera_validation_rejects_a_missing_expected_camera(self) -> None:
        ...
        prediction_input = SimpleNamespace(
            ...
            acceleration=0.0,
            ego_pose_history=[SimpleNamespace(timestamp_us=1000, x=0.0, y=0.0, yaw=0.0)],
            route_waypoints=[{"x": 0.0, "y": 0.0, "z": 0.0}, {"x": 20.0, "y": 0.0, "z": 0.0}],
            alpasignal={"hazards": []},
        )
        ...
        self.assertIn("camera_front_left_45fov", str(ctx.exception))

    def test_baseline_driver_rejects_frozen_multi_camera_content_when_timestamps_advance(self) -> None:
        # Mirrors test_direct_actor_planner_rejects_frozen_camera_content_when_timestamps_advance
        # but with two cameras, proving the combined freshness fingerprint genuinely
        # incorporates every camera rather than only the first one in the dict.
        model = RouteFollowingAlpaSimModel(
            camera_ids=["camera_front_wide_120fov", "camera_front_left_45fov"],
            context_length=1,
            output_frequency_hz=4,
        ...
        def _frame(*, ts: int, x: float) -> SimpleNamespace:
            return SimpleNamespace(
                camera_images={
                    "camera_front_wide_120fov": [
                        SimpleNamespace(timestamp_us=ts, image=np.full((4, 4, 3), 180, dtype=np.uint8))
                    ],
                    "camera_front_left_45fov": [
                        SimpleNamespace(timestamp_us=ts, image=np.full((4, 4, 3), 90, dtype=np.uint8))
                    ],
                },
                command=DriveCommand.STRAIGHT,
                speed=4.0,
                acceleration=0.0,
                ego_pose_history=[SimpleNamespace(timestamp_us=ts, x=x, y=0.0, yaw=0.0)],
                route_waypoints=[{"x": 0.0, "y": 0.0, "z": 0.0}, {"x": 20.0, "y": 0.0, "z": 0.0}],
                alpasignal={"hazards": []},
            )

        model.predict(_frame(ts=1000, x=0.0))
        with self.assertRaises(RuntimeError) as ctx:
            # Both cameras' image content is byte-identical to the first call while
            # the ego pose and timestamp advance -- this must still be caught.
            model.predict(_frame(ts=1100, x=1.0))
...
def _skip_torch_dependent_tests_if_needed() -> None:
    if os.getenv("ALPABRIDGE_CORE_CONFORMANCE") == "1":
