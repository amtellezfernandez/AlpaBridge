...
SRC = ROOT / "src"
SCRIPT = ROOT / "scripts" / "alpabridge_doctor.py"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))


def _load_module():
    return importlib.import_module("alpabridge.cli.commands.alpabridge_doctor")


def _load_script_module():
    spec = importlib.util.spec_from_file_location("alpabridge_doctor_script", SCRIPT)
...
def _clean_alpabridge_dist(module) -> FakeDist:
    alpabridge_console_scripts = [
        SimpleNamespace(name=name, group="console_scripts", value=f"alpabridge.cli:{name}")
        for name in module.EXPECTED_CONSOLE_SCRIPTS
    ]
    alpabridge_model_entry_points = [
        SimpleNamespace(
            name="constant_velocity",
            group="alpasim.models",
            value="alpabridge.simulator.baseline_drivers:ConstantVelocityAlpaSimModel",
        ),
        SimpleNamespace(
        ...
        SimpleNamespace(
            ...
            group="alpasim.models",
            value="alpabridge.simulator.alpasim_direct_actor_planner:DirectActorPlannerAlpaSimModel",
        ),
    ]
    return FakeDist("alpabridge", alpabridge_console_scripts + alpabridge_model_entry_points)


class AlpaBridgeDoctorTests(unittest.TestCase):
    def test_build_report_validates_public_release_surface(self) -> None:
        module = _load_module()
        alpabridge_dist = _clean_alpabridge_dist(module)

        with patch.object(module, "distribution", return_value=alpabridge_dist), patch.object(
            module, "distributions", return_value=[alpabridge_dist]
        ):
        ...
        self.assertTrue(report["checks"]["public_model_entry_points_owned_by_alpabridge"])
        self.assertTrue(
    ...
    def test_build_report_flags_duplicate_model_provider_conflicts(self) -> None:
        ...
        alpabridge_dist = FakeDist("alpabridge", alpabridge_console_scripts + alpabridge_model_entry_points)
        ...
        with patch.object(module, "distribution", return_value=alpabridge_dist), patch.object(
            module, "distributions", return_value=[alpabridge_dist, competing_dist]
        ):
            report = module.build_report()

        self.assertFalse(report["valid"])
        self.assertFalse(report["checks"]["public_model_entry_points_unique"])
        self.assertFalse(report["checks"]["public_model_entry_points_owned_by_alpabridge"])
        self.assertEqual(
            [
                "token_dagger_bc: alpabridge -> alpabridge.simulator.alpasim_token_bc:TokenBCAlpaSimModel, "
    ...
    def test_human_report_surfaces_environment_conflicts(self) -> None:
        ...
        report["conflicts"] = {
            "duplicate_public_model_entry_points": [
                "token_dagger_bc: alpabridge -> alpabridge.simulator.alpasim_token_bc:TokenBCAlpaSimModel, another-package -> another_package.token_bc:TokenBCAlpaSimModel"
            ],
        ...
        self.assertIn("rerun alpabridge-setup", rendered)
    ...
    def test_script_can_write_json_report(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual("alpabridge_doctor_v1", payload["schema"])
