...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate contract diagnostics against controlled mutations of retained "
            "external AlpaSim telemetry."
        )
    )
    ...
    parser.add_argument("--guard-iterations", type=int, default=200)
    parser.add_argument("--guard-batch-size", type=int, default=20)
...
def main() -> int:
    ...
    result = run_diagnostic_experiment(
        ...
        guard_iterations=args.guard_iterations,
...
def _write_cases(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for row in rows:
            serialized = dict(row)
            serialized["wod2sim_observed_codes"] = ";".join(
                serialized["wod2sim_observed_codes"]
            )
...
def _console_summary(result: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "online_guard_overhead": result["online_guard_overhead"],
