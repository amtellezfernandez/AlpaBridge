...
def run_release_bootstrap_smoke(*, source_root: Path, keep_temp: bool, installer: str = "auto") -> dict[str, Any]:
    ...
    temp_dir = tempfile.mkdtemp(prefix="alpabridge-bootstrap-")
    ...
    step_specs = [
        *_bootstrap_install_steps(
        ...
        (
            "installed_doctor",
            [
                str(venv_bin / "alpabridge-doctor"),
                "--strict-installed",
                "--json",
                "--output",
                str(doctor_json),
        ...
        (
            ...
            [
                str(venv_python),
                "scripts/alpabridge_doctor.py",
                "--json",
                "--output",
                str(source_doctor_json),
            ],
        ),
        ("launch_help", [str(venv_bin / "alpabridge-launch"), "--help"]),
        ("setup_help", [str(venv_bin / "alpabridge-setup"), "--help"]),
        ("ready_help", [str(venv_bin / "alpabridge-ready"), "--help"]),
        ("local_cache_help", [str(venv_bin / "alpabridge-build-local-cache"), "--help"]),
        ("oracle_proxy_help", [str(venv_bin / "alpabridge-build-oracle-proxy"), "--help"]),
        ("audit_run_help", [str(venv_bin / "alpabridge-audit-run"), "--help"]),
        ("support_bundle_help", [str(venv_bin / "alpabridge-support-bundle"), "--help"]),
        ("reproduce_help", [str(venv_bin / "alpabridge-reproduce"), "--help"]),
        ("benchmark_summary_help", [str(venv_bin / "alpabridge-benchmark-summary"), "--help"]),
        ("benchmark_readiness_help", [str(venv_bin / "alpabridge-benchmark-readiness"), "--help"]),
        (
            "reproduce_plan",
            [
                str(venv_bin / "alpabridge-reproduce"),
                "--scene-id",
                "bootstrap-scene",
                "--run-dir",
        ...
        (
            ...
            [
                str(venv_bin / "alpabridge-benchmark-summary"),
                "--evidence-dir",
                str(reproduce_dir / "evidence"),
                "--output",
    ...
    report = {
        "schema": "alpabridge_release_bootstrap_smoke_v1",
        "valid": ok,
