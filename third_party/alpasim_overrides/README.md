...
| `arm64-docker-build.patch` | no | Dockerfile install line moved and became `--extra all --extra recipes`, and upstream has since adopted the multi-stage arch split itself. Also has an uncaptured fix to fold in (CUDA keyring repo from `/etc/os-release`) — see that `.md`. Re-verification needs another real ARM64 `docker build` |
