...
3. **The scoping bug, mechanism pinned by four minimal-repro probes:** BuildKit's builtin platform args (`TARGETARCH` etc.) are always available to `FROM` lines — `FROM base-${TARGETARCH}` resolves with no `ARG` anywhere. An `ARG` line appearing after a `FROM` belongs to *that stage*, and is visible to `RUN` only in that stage and in stages that derive from it. The existing `ARG TARGETARCH` sits between the dcgm-exporter `FROM` and the final `FROM` — i.e. inside the dcgm-exporter stage — and the final stage derives from `base-amd64`/`base-arm64`, not from dcgm-exporter, so `RUN` steps there see the empty string. Probes: global ARG → empty; no ARG → FROM still resolves, RUN empty; ARG in an ancestor stage → inherited; ARG in a non-ancestor stage (this Dockerfile's shape) → empty. An earlier revision of this document attributed the behaviour to "three or more named stages"; stage count is irrelevant — ancestry is the rule. Fix unchanged: re-declare `ARG TARGETARCH` immediately after the final `FROM`.
